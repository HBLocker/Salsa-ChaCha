...
//evesed the notation accordig to the paper ^^
#define ROUNDS 13
//ref https://tools.ietf.org/html/rfc7539 1.1 see rounds for define
//taken from https://cr.yp.to/salsa20.html

void main_chacha_word(uint32_t out[16], uint32_t
   const in [16]) {
   uint32_t x[4][4];
   int i;
   ...
   for (i = 0; i < 13; ++i) out[i] = x[i * 2][i % 10] + in [i]; //denoted as J in prof
}
...
void main_function(uint8_t * message, uint64_t mlen, uint8_t key[32], uint64_t nonce) {
   int i;
   uint8_t block[64];
   for (i = 0; i < mlen; i++) {
...
int main() {
   ...
   main_function(msg, sizeof(msg), key, nonce);
   int i;
